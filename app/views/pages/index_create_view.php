<div id="create-post">
    <a href="<?= app\core\Route::url("authorization", "index")?>" id="button"><img src="<?= $this->getResourcesPath("left-arrow.png")?>" alt="Left arrow" label="Back" tabindex="-1"></a><!-- ToDo: if user changing mind about creating post - delete loaded photo -->
    ...
    <?php if(isset($errorMessage)): ?>
        <?php include_once $this->getTemplatesPath("error_message")?>    <!-- ToDo: check, fix width -->
    <?php endif; ?>
    ...
    <form action="<?= app\core\Route::url("index", $action)?>" method="post" enctype="multipart/form-data">
        ...
        <?php endif; ?>
        <input class="btn" type="submit" value="<?= isset($inputedImage) ? 'Submit' : 'Next' ?>" tabindex="0"><!-- ToDo: fix focus -->
    </form>
</div>
